...
{
    /// <summary>
    /// 提供“修改过渡”模式下的实际 AnimatorController 操作逻辑。
    /// </summary>
    internal static class QuickTransitionModifyService
    {
        ...
        internal struct ConditionSetting
        {
            ...
            public IntIncrementDirection incrementDirection;
            ...
            IReadOnlyList<ConditionSetting> conditionSettings)
        {
            ...
            if (modifyMode == ModifyMode.FromStateTransitions)
            {
                ...
                {
                    var state = FindStateByPath(stateMachine, stateName);
            ...
            {
                Debug.LogWarning("[QuickTransition] 未找到符合条件的过渡可修改");
                return;
            }
            ...
            if (conditionSettings != null)
            {
                for (int i = 0; i < conditionSettings.Count; i++)
                {
                    ...
                    if (param.type == AnimatorControllerParameterType.Int)
                    {
                        ...
                        {
                            // 改为连接顺序
                            condSetting.sortMode = SortMode.ArrangementOrder;
                        }
                        else
                        {
            ...
            if (conditionSettings != null && stateName != "Exit" && conditionSettings.Any(c => c.enableIntAutoIncrement && c.sortMode == SortMode.NameNumberOrder))
            {
                if (modifyMode == ModifyMode.FromStateTransitions)
                {
            ...
            if (conditionSettings != null)
            {
                ...
                {
                    var condSetting = conditionSettings[i];
                    if (!condSetting.enableIntAutoIncrement || condSetting.incrementDirection != IntIncrementDirection.Decrement)
                    {
                        continue;
                    }
                ...
                if (modifyConditions && conditionSettings != null)
                {
                    ...
                    for (int i = 0; i < conditionSettings.Count; i++)
                    {
                        ...
                        if (condSetting.enableIntAutoIncrement)
                        {
                            ...
                            {
                                if (condSetting.incrementDirection == IntIncrementDirection.Increment)
                                {
            ...
            Debug.Log($"[QuickTransition] 成功修改了 {transitionsToModifyInfo.Count} 个过渡");
        }

        private static AnimatorState FindState(AnimatorStateMachine stateMachine, string name)
        {
            if (stateMachine == null || string.IsNullOrEmpty(name)) return null;
