...
namespace MVA.Toolbox.AvatarQuickToggle.Editor
{
    ...
    public class QuickToggleConfigEditor : UnityEditor.Editor
    {
        ...
        private void DrawBoolSection(ref Rect rect, SerializedProperty defaultState, SerializedProperty createMenu,
            SerializedProperty menuPath, SerializedProperty menuName, SerializedProperty targets)
        {
            ...
            if (createMenu.boolValue)
            {
                menuPath.stringValue = EditorGUI.TextField(rect, "菜单项位置", menuPath.stringValue);
                rect.y += line + 2f;
                menuName.stringValue = EditorGUI.TextField(rect, "菜单项名称", menuName.stringValue);
                rect.y += line + 4f;
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var targetObj = item.FindPropertyRelative("targetObject");
                var controlType = item.FindPropertyRelative("controlType");
                var blendShapeName = item.FindPropertyRelative("blendShapeName");
                var goState = item.FindPropertyRelative("goState");
                var bsState = item.FindPropertyRelative("bsState");
                ...
                Rect r = rect;
                r.height = line;
                EditorGUI.ObjectField(r, targetObj, new GUIContent("目标对象"));
                rect.y += line + 2f;

                r = rect;
                r.width *= 0.5f;
                controlType.intValue = EditorGUI.Popup(r, "模式", controlType.intValue, new[] { "GameObject", "BlendShape" });
                r.x += r.width + 4f;
        ...
        private void DrawIntSection(ref Rect rect, SerializedProperty defaultInt, SerializedProperty createMenu,
            SerializedProperty subMenuName, SerializedProperty menuItemNames, SerializedProperty intGroups)
        {
            float line = EditorGUIUtility.singleLineHeight;
            ...
            if (createMenu.boolValue)
            {
                ...
                if (menuItemNames.arraySize == 0)
                    menuItemNames.arraySize = 1;

                for (int i = 0; i < menuItemNames.arraySize; i++)
                {
                    var nameProp = menuItemNames.GetArrayElementAtIndex(i);
                    nameProp.stringValue = EditorGUI.TextField(rect, $"  项 {i}", nameProp.stringValue);
                    rect.y += line + 2f;
                }
            }
            ...
            if (intGroups.arraySize == 0)
                intGroups.arraySize = 1;

            for (int g = 0; g < intGroups.arraySize; g++)
            {
                var group = intGroups.GetArrayElementAtIndex(g);
                var stateNameProp = group.FindPropertyRelative("stateName");
                var targetsProp = group.FindPropertyRelative("targetItems");

                Rect r = rect;
                r.height = line;
                stateNameProp.stringValue = EditorGUI.TextField(r, $"组 {g} 名称", stateNameProp.stringValue);
                rect.y += line + 2f;
                ...
                {
                    var item = targetsProp.GetArrayElementAtIndex(i);
                    var targetObj = item.FindPropertyRelative("targetObject");
                    var controlType = item.FindPropertyRelative("controlType");
                    var blendShapeName = item.FindPropertyRelative("blendShapeName");
                    var goState = item.FindPropertyRelative("goState");
        ...
        private void DrawFloatSection(ref Rect rect, SerializedProperty defaultFloat, SerializedProperty createMenu,
            SerializedProperty menuPath, SerializedProperty menuName, SerializedProperty targets)
        {
            float line = EditorGUIUtility.singleLineHeight;
            ...
            for (int i = 0; i < targets.arraySize; i++)
            {
                ...
                direction.intValue = EditorGUI.Popup(rect, "方向", direction.intValue, new[] { "0->100", "100->0" });
                rect.y += line + 2f;

                splitBlend.boolValue = EditorGUI.ToggleLeft(rect, "二分模式", splitBlend.boolValue);
                rect.y += line + 2f;

                if (splitBlend.boolValue)
                {
                    secondaryName.stringValue = EditorGUI.TextField(rect, "BlendShape2", secondaryName.stringValue);
                    rect.y += line + 2f;
                    secondaryDir.intValue = EditorGUI.Popup(rect, "方向2", secondaryDir.intValue, new[] { "0->100", "100->0" });
                    rect.y += line + 2f;
                }
