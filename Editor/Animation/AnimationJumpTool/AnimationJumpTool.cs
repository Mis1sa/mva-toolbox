...
namespace MVA.Toolbox.AnimationJumpTool
{
    /// <summary>
    /// AnimationClip 跳转工具：从剪辑定位到场景中的 Animator 或 Avatar
    /// </summary>
    public static class AnimationJumpTool
    {
        ...
        [MenuItem("Assets/MVA Toolbox/跳转到动画界面", false, 101)]
        ...
        [MenuItem("Assets/MVA Toolbox/跳转到动画界面", true)]
        ...
        public static bool TryJumpToClip(AnimationClip clip)
        {
            ...
            {
                Debug.LogWarning($"[AnimationJumpTool] 跳转过程中发生异常: {ex.Message}");
            }
        ...
        private static bool ForceAnimationWindowToShowClip(AnimationClip clip, GameObject targetGameObject)
        {
            ...
            {
                Debug.LogWarning($"[AnimationJumpTool] 打开 Animation 窗口失败: {ex.Message}");
                return false;
            }
        }
