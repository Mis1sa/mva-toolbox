...
{
    internal static class AnimatorStateSplitService
    {
            ...
            List<TransitionAdjustment> tailAdjustments)
        {
            ...
            {
                Debug.LogError("[AnimatorStateSplitService] Execute: 无效的输入参数。");
                return false;
            }

            if (isOriginalHead && isOriginalTail)
            {
                Debug.LogError("[AnimatorStateSplitService] Execute: 原始状态不能同时作为头部和尾部。");
                return false;
            }
            ...
            {
                Debug.LogError($"[AnimatorStateSplitService] Execute: 无法在 {layer.name} 层级中找到状态 {originalState.name}。");
                return false;
            }
            ...
            Undo.RecordObject(controller, "Animator State - Split Animator State");
            Undo.RecordObject(parentStateMachine, "Animator State - Split Animator State Machine");
            ...
            if (!isOriginalTheHead)
            {
                ...
                Undo.RecordObject(tailState, "Animator State - Rename Original State to Tail");
                tailState.name = tailStateName;
            }
            else
            {
                headState = originalState;
                Undo.RecordObject(headState, "Animator State - Rename Original State to Head");
            ...
            if (headState == originalState)
            {
                ...
                Undo.RecordObject(originalState, "Animator State - Clear Head State Outgoing");
            ...
            if (tailAdjustments != null)
            {
                foreach (var adj in tailAdjustments.Where(a => a.ShouldMove))
                {
                    ...
                    if (tailTransition != null)
                    {
                        ...
                        Undo.RecordObject(headState, "Animator State - Move Transition To Head");
                        ...
                        Undo.RecordObject(tailState, "Animator State - Remove Transition From Tail");
        ...
        internal static List<AnimatorState> GetAllStatesInLayer(AnimatorController controller, int layerIndex)
        {
        ...
        private static bool FindStateAndParentMachine(AnimatorStateMachine root, AnimatorState targetState, ref Vector3 position, ref AnimatorStateMachine parentMachine)
        {
            ...
            {
                if (sub.stateMachine != null && FindStateAndParentMachine(sub.stateMachine, targetState, ref position, ref parentMachine))
                {
                    return true;
                }
            }
