...
- FBX替换：新增基于临时工作区的 FBX 对齐与替换工具，支持层级、命名、骨骼、Renderer、材质与组件的分阶段确认。

## [1.0.0] - 2026-05-22

### Changed

- 动画工具：统一动画查询、动画重定向与默认值烘焙在 MMA 控制器下的范围解析与高亮逻辑。
- 移除网格权重骨骼：移除子级非骨骼物体相关选项，固定仅清理独占骨骼并保留子级释放逻辑。

### Fixed

- 动画工具：修复 MA Merge Animator、根物体、子 Animator 与全部控制器模式下的多处范围判断异常，并修复 Switch Generator 在控制器筛选下的显示问题。
