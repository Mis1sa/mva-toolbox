...
{
    internal static class AnimatorParameterTraceService
    {
        ...
        internal static TraceResult Execute(
            ...
            GameObject sceneRoot)
        {
            ...
            RegisterControllerUsages(controller, parameterName, result, referenceDedup, modificationDedup);
            RegisterAnimationCurveModifications(controller, parameterName, result, modificationDedup);
            RegisterSceneComponentModifications(sceneRoot, parameterName, result, modificationDedup);
            ...
            HashSet<string> modificationDedup)
        {
            ...
            SerializedProperty parameterProp = serializedObject.FindProperty("parameter");
            ...
            HashSet<string> modificationDedup)
        {
            ...
            for (int i = 0; i < states.Length; i++)
            {
                ...
                AnalyzeState(state, layerName, sourceState, parameterName, result, referenceDedup, modificationDedup);
            ...
            for (int i = 0; i < anyStateTransitions.Length; i++)
            {
                ...
                string destinationState = ResolveDestinationState(transition, stateMachinePath);
                AnalyzeTransition(transition, layerName, sourceState, destinationState, parameterName, result, referenceDedup);
            }

            AnimatorTransition[] entryTransitions = stateMachine.entryTransitions;
            for (int i = 0; i < entryTransitions.Length; i++)
            {
                AnimatorTransition transition = entryTransitions[i];
            ...
            HashSet<string> modificationDedup)
        {
            ...
            {
                AddReference(result, referenceDedup, TraceSourceKind.StateParameter, "State Speed 参数", state,
                    layerName: layerName, sourceState: sourceState, propertyPath: "speedParameter");
            }

            if (state.mirrorParameterActive && string.Equals(state.mirrorParameter, parameterName, StringComparison.Ordinal))
            {
                AddReference(result, referenceDedup, TraceSourceKind.StateParameter, "State Mirror 参数", state,
                    layerName: layerName, sourceState: sourceState, propertyPath: "mirrorParameter");
            }

            if (state.cycleOffsetParameterActive && string.Equals(state.cycleOffsetParameter, parameterName, StringComparison.Ordinal))
            {
                AddReference(result, referenceDedup, TraceSourceKind.StateParameter, "State Cycle Offset 参数", state,
                    layerName: layerName, sourceState: sourceState, propertyPath: "cycleOffsetParameter");
            }

            if (state.timeParameterActive && string.Equals(state.timeParameter, parameterName, StringComparison.Ordinal))
            {
                AddReference(result, referenceDedup, TraceSourceKind.StateParameter, "State Time 参数", state,
                    layerName: layerName, sourceState: sourceState, propertyPath: "timeParameter");
            }

            if (state.motion is BlendTree blendTree)
            {
                AnalyzeBlendTree(blendTree, layerName, sourceState, blendTree.name, parameterName, result, referenceDedup);
            }
            ...
            for (int i = 0; i < behaviours.Length; i++)
            {
                AnalyzeBehaviour(behaviours[i], layerName, sourceState, parameterName, result, referenceDedup, modificationDedup);
            }
        }
            ...
            HashSet<string> referenceDedup)
        {
            ...
            {
                AddReference(result, referenceDedup, TraceSourceKind.BlendTreeParameter, "BlendTree 参数 (X/1D)", blendTree,
                    layerName: layerName, sourceState: sourceState, blendTreePath: currentBlendTreePath, propertyPath: "blendParameter");
            }
            ...
            {
                AddReference(result, referenceDedup, TraceSourceKind.BlendTreeParameter, "BlendTree 参数 (Y)", blendTree,
                    layerName: layerName, sourceState: sourceState, blendTreePath: currentBlendTreePath, propertyPath: "blendParameterY");
            }
            ...
            for (int i = 0; i < children.Length; i++)
            {
                ...
                {
                    AddReference(result, referenceDedup, TraceSourceKind.BlendTreeParameter, "Direct BlendTree 子项参数", blendTree,
                        layerName: layerName, sourceState: sourceState, blendTreePath: currentBlendTreePath,
                ...
                if (child.motion is BlendTree childTree)
                {
                    ...
                    AnalyzeBlendTree(childTree, layerName, sourceState, childBlendTreePath, parameterName, result, referenceDedup);
                }
            }
        }
            ...
            HashSet<string> referenceDedup)
        {
            ...
            for (int i = 0; i < conditions.Length; i++)
            {
                ...
                AddReference(result, referenceDedup, TraceSourceKind.TransitionCondition, "Transition Condition 参数", transition,
                    layerName: layerName, sourceState: sourceState, destinationState: destinationState,
            ...
            HashSet<string> modificationDedup)
        {
            ...
            for (int i = 0; i < parametersProp.arraySize; i++)
            {
                ...
                {
                    AddModification(result, modificationDedup, TraceSourceKind.AvatarParameterDriverTarget, "Avatar Parameter Driver 写入参数", behaviour,
                        layerName: layerName, sourceState: sourceState, componentName: behaviour.GetType().Name,
                ...
                {
                    AddReference(result, referenceDedup, TraceSourceKind.AvatarParameterDriverSource, "Avatar Parameter Driver 读取参数", behaviour,
                        layerName: layerName, sourceState: sourceState, componentName: behaviour.GetType().Name,
            ...
            HashSet<string> modificationDedup)
        {
            ...
            foreach (AnimationClip clip in clips)
            {
                ...
                for (int i = 0; i < bindings.Length; i++)
                {
                    ...
                    if (!TryExtractAnimatorParameterName(binding.propertyName, out string curveParameter))
                    {
                        continue;
                    }
                ...
                if (matched)
                {
                    AddModification(result, modificationDedup, TraceSourceKind.AnimationCurve, "动画剪辑写入参数", clip);
                }
            }
        }
            ...
            HashSet<string> modificationDedup)
        {
            ...
            Component[] components = sceneRoot.GetComponentsInChildren<Component>(true);
            for (int i = 0; i < components.Length; i++)
            {
                ...
                if (isPhysBone && TryMatchPhysBoneWrite(component, parameterName, out string suffix))
                {
                    AddModification(result, modificationDedup, TraceSourceKind.SceneComponent, "场景组件写入参数", component,
                        hierarchyPath: GetRelativeHierarchyPath(sceneRoot.transform, component.transform),
                        componentName: component.GetType().Name,
                ...
                if (isKnownWriter && TryRegisterKnownSceneWriterModification(component, sceneRoot, parameterName, result, modificationDedup))
                {
                    continue;
                }
                ...
                while (iterator.NextVisible(enterChildren))
                {
                    ...
                    string propertyPath = iterator.propertyPath ?? string.Empty;
        ...
        private static bool TryMatchPhysBoneWrite(Component component, string parameterName, out string suffix)
        {
            ...
            for (int i = 0; i < PhysBoneSuffixes.Length; i++)
            {
                string currentSuffix = PhysBoneSuffixes[i];
        ...
        {
            AddEntry(result.References, dedup, sourceKind, description, relatedObject, layerName, sourceState,
                destinationState, componentName, blendTreePath, hierarchyPath, propertyPath);
        }
        ...
        private static void AddModification(
            ...
            string propertyPath = null)
        {
            AddEntry(result.Modifications, dedup, sourceKind, description, relatedObject, layerName, sourceState,
                destinationState, componentName, blendTreePath, hierarchyPath, propertyPath);
        }
