...
namespace MVA.Toolbox.SwitchGenerator.Authoring
{
    [CustomEditor(typeof(SwitchGeneratorConfig))]
    public class SwitchGeneratorConfigEditor : UnityEditor.Editor
    {
        ...
        {
            _configList = new ReorderableList(serializedObject, _layersProp, true, true, true, true)
            {
            ...
            {
                int index = _layersProp.arraySize;
                ...
                element.FindPropertyRelative("switchType").enumValueIndex = 0;
            ...
            {
                if (list.index < 0 || list.index >= _layersProp.arraySize) return;
                _layersProp.DeleteArrayElementAtIndex(list.index);
                if (list.index >= _layersProp.arraySize)
                    list.index = _layersProp.arraySize - 1;
            };
        }

        private static void EnsureArrayInitialized(SerializedProperty arrayProp)
        {
        ...
        private void DrawConfigList()
        {
            ...
            if (_configList.index >= 0 && _configList.index < _layersProp.arraySize)
            {
                var element = _layersProp.GetArrayElementAtIndex(_configList.index);
                EditorGUILayout.Space();
        ...
        private void DrawLayerConfigDetail(SerializedProperty layerConfig)
        {
            ...
            var switchTypeProp = layerConfig.FindPropertyRelative("switchType");
            ...
            var writeDefaultsProp = layerConfig.FindPropertyRelative("writeDefaults");
            var generateMenuControlProp = layerConfig.FindPropertyRelative("generateMenuControl");
            var menuPathProp = layerConfig.FindPropertyRelative("menuPath");
            var boolMenuItemNameProp = layerConfig.FindPropertyRelative("boolMenuItemName");
            var floatMenuItemNameProp = layerConfig.FindPropertyRelative("floatMenuItemName");
            ...
            var defaultBoolProp = layerConfig.FindPropertyRelative("defaultBoolValue");
            ...
            EditorGUILayout.PropertyField(writeDefaultsProp, new GUIContent("Write Defaults 设置"));

            EditorGUILayout.Space();
            EditorGUILayout.PropertyField(switchTypeProp, new GUIContent("开关类型"));

            EditorGUILayout.Space();
            switch (switchTypeProp.enumValueIndex)
            {
                case 0:
                    DrawBoolSection(defaultBoolProp, generateMenuControlProp, menuPathProp, boolMenuItemNameProp, boolTargetsProp);
                    break;
                case 1:
                    DrawIntSection(defaultIntProp, generateMenuControlProp, intSubMenuNameProp, intMenuItemNamesProp, intGroupsProp);
                    break;
                case 2:
                    DrawFloatSection(defaultFloatProp, generateMenuControlProp, menuPathProp, floatMenuItemNameProp, floatTargetsProp);
                    break;
            }
        }

        private static void DrawNameWithOverwrite(string label, SerializedProperty valueProp, SerializedProperty overwriteProp)
        {
        ...
        private static void DrawMenuOptions(SerializedProperty createMenu, SerializedProperty menuPath, SerializedProperty menuName)
        {
            createMenu.boolValue = EditorGUILayout.ToggleLeft("生成菜单项", createMenu.boolValue);
            if (createMenu.boolValue)
            {
        ...
        private void DrawTargetList(SerializedProperty listProp, TargetMode mode)
        {
            ...
            for (int i = 0; i < listProp.arraySize; i++)
            {
                ...
                targetObjProp.objectReferenceValue = TargetObjectResolver.ResolveValidTarget(newGo, avatar);

                if (mode == TargetMode.Bool)
                {
                    var controlType = item.FindPropertyRelative("controlType");
                    var boolObjectState = item.FindPropertyRelative("boolObjectState");
                    var boolBlendShapeState = item.FindPropertyRelative("boolBlendShapeState");
                    var blendShapeName = item.FindPropertyRelative("blendShapeName");

                    EditorGUILayout.PropertyField(controlType, new GUIContent("模式"));
                    if (controlType.enumValueIndex == 0)
                    {
                        EditorGUILayout.PropertyField(boolObjectState, new GUIContent("状态"));
                    }
                    else
                    {
                        blendShapeName.stringValue = EditorGUILayout.TextField("BlendShape", blendShapeName.stringValue);
                        EditorGUILayout.PropertyField(boolBlendShapeState, new GUIContent("状态"));
                    }
                }
                else
                {
                    var blendShapeName = item.FindPropertyRelative("blendShapeName");
                    var floatDirection = item.FindPropertyRelative("floatDirection");
                    ...
                    var secondaryDir = item.FindPropertyRelative("secondaryFloatDirection");

                    blendShapeName.stringValue = EditorGUILayout.TextField("BlendShape", blendShapeName.stringValue);
                    EditorGUILayout.PropertyField(floatDirection, new GUIContent("方向"));
                    ...
                    if (splitBlend.boolValue)
                    {
                        ...
                        EditorGUILayout.PropertyField(secondaryDir, new GUIContent("方向2"));
                    }
                }
...
#if AAO_VRCSDK3_AVATARS && MVA_AAO_SUPPORT
    [ComponentInformation(typeof(SwitchGeneratorConfig))]
    internal sealed class SwitchGeneratorConfigComponentInformation : ComponentInformation<SwitchGeneratorConfig>
    {
        protected override void CollectDependency(SwitchGeneratorConfig component, ComponentDependencyCollector collector)
        {
